locals {
  ...
  ec2_zone_mapping = { "${local.preferred_az}" : { "subnet_id" : "${var.subnet_id}", "security_groups" : [aws_security_group.ec2_security_group.name] } }
  ...
  auto_ha_availability_zonea = {
    "${data.aws_region.current.name}a" : {
      ...
      "security_groups" : ["subnet-0f8e8d1312d4b9cc9"]
  } }
